...
from rdflib.namespace import RDF, RDFS  # import predefined RDF and RDFS namespaces
import warnings  # import warnings module for issuing warning messages
...
def tei_rdfa(xmlfile: str, xpath_expr: Optional[str] = None, verbose: bool = True) -> Graph:  # define function
    ...
    try:
        ...
        # create RDF graph
        g = Graph()  # initialize empty RDF graph object
...
def process_xpath_elements(tree: etree.ElementTree, graph: Graph, xpath_expr: str, 
    ...
    try:
        ...
        # issue warning
        if not elements:
            warning_msg = f'No elements matching XPath expression "{xpath_expr}" detected in the document.'
        ...
        # log result
        if verbose:
            logger.info(f'{len(elements)} element(s) matching XPath expression "{xpath_expr}" detected.')
...
def get_tei_prefixes(tree: etree.ElementTree, namespaces: Dict[str, str]) -> Dict[str, str]:  # define function
    ...
    # add predefined RDF namespaces to defaultdict object
...
def determine_subject(element: etree.Element, parent_subject: Optional[str], 
    ...
    # check presence of "resource" attribute (when no "rel"/"rev" attributes are present)
...
def process_property(element: etree.Element, subject: str, graph: Graph, prefix_map: Dict[str, str]) -> None:  # define function
    ...
    for prop in properties:  # iterate through "property" values
        ...
        obj_value = determine_property_object(element, prefix_map)  # call determine_property_object() helper function, return object value (or None)
        
        # add triple to RDF graph if object value unequal None
        if obj_value:
...
def determine_property_object(element: etree.Element, prefix_map: Dict[str, str]) -> Optional[Union[URIRef, Literal]]:  # define function
    ...
    if 'resource' in element.attrib:
        return URIRef(expand_uri(element.attrib['resource'], prefix_map))  # call expand_uri() helper function, return full "resource" URI as URIRef object
    ...
    # check presence of element content (excluding empty string)
    if element.text and element.text.strip():
        return Literal(element.text.strip())  # return element content (without leading/trailing whitespace) as Literal
        
    text_parts = [child.text.strip() for child in element  # iterate through direct child elements, return element content (withouth leading/trailing whitespace) 
                 if child.text and child.text.strip()]  # check presence of child-element content (excluding empty string)
